...
namespace Nbugs.OperationApp.UI.Admin.Core
{
    ...
    {
        private long minMum { get; set; }

        private long maxMum { get; set; }
    ...
    {
        private static string regStr = @"^[0-9A-Za-z_-]{0,}$";
    ...
    {
        private static string regStr = @"^[\u4e00-\u9fa5]{0,}$";
    ...
    {
        private static string regStr = @"^[-]?\d+(\.\d+)?$";
    ...
    {
        private static string regStr = @"^\w+((-\w+)|(\.\w+))*\@[A-Za-z0-9]+((\.|-)[A-Za-z0-9]+)*\.[A-Za-z0-9]+$";
    ...
    {
        private static string regStr = @"^((0\d{2,5}-)|\(0\d{2,5}\))?\d{7,8}(-\d{3,4})?$";
    ...
    {
        private static string regStr = @"http(s)?://([\w-]+\.)+[\w-]+(/[\w- ./?%&=]*)?";
